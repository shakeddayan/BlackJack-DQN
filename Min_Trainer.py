...
def init_players(resume_training, path_to_load):
    # Initialize player
    ...
    return player, player_hat


def get_action_reward(action, p_sum, d_card_val, env, counters):
    '''
    Returns the shaped reward for a given action and updates action counters.
    counters is a dict with keys: hits, doubles, splits, stands, surrenders, overall.
    Also returns done=True if action is surrender.
    '''
    reward = 0
    done = False
    hits, doubles, splits, stands, surrenders, overall = (
        counters['hits'], counters['doubles'], counters['splits'],
        counters['stands'], counters['surrenders'], counters['overall']
    )
    ...
    counters['hits'] = hits
    counters['doubles'] = doubles
    counters['splits'] = splits
    counters['stands'] = stands
    counters['surrenders'] = surrenders
    counters['overall'] = overall

    return reward, done


def run_episode(env, player, epoch, epoches, start_epoch, buffer, counters, win, current_win_count, totalR):
    '''
    Runs a single training episode (one hand of blackjack).
    Returns updated win, current_win_count, totalR, episode_reward, has_doubled.
    '''
    done = False
    has_doubled = False
    episode_reward = 0 
    ...
    while not done:
        ...
        reward, done = get_action_reward(action, p_sum, d_card_val, env, counters)
    ...
    return win, current_win_count, totalR, episode_reward, has_doubled


def train_step(player, player_hat, buffer, batch_size, optim, gamma=0.99):
    '''
...
def log_and_reset(epoch, start_epoch, player, env, counters, running_loss, loss_count, running_reward, current_win_count, calc_win_from):
    '''
    ...
    wandb.log({
        ...
        "action_stats/hits": counters['hits'],
        "action_stats/stands": counters['stands'],
        "action_stats/doubles": counters['doubles'],
        "action_stats/surrenders": counters['surrenders']
    })
...
def save_model(player, run_name):
    # Save model artifact
...
def run_test_phase(env, player, test_epoches=100000):
    '''
...
def main():
    ...
    config = {
        ...
        "epoches": EPOCH_START + 1500000, #make sure if resuming this is the old + new together
        "batch_size": 128,
        "train_batch_freq": 10,
    ...
    player, player_hat = init_players(RESUME_TRAINING, PATH_TO_LOAD)
    ...
    for epoch in range(start_epoch, epoches):
        ...
        win, current_win_count, totalR, episode_reward, _ = run_episode(
            env, player, epoch, epoches, start_epoch, buffer, counters, win, current_win_count, totalR
        )
        ...
        if len(buffer) > batch_size and epoch % train_batch == 0:
            loss_val = train_step(player, player_hat, buffer, batch_size, optim, gamma= config["gamma"])
        ...
        if epoch > 0 and epoch % calc_win_from == 0:
            running_loss, loss_count, running_reward, current_win_count, win = log_and_reset(
                epoch, start_epoch, player, env, counters,
    ...
    print("PERCENTAGES")
    if counters['overall'] > 0:
        print(f'hit: {100*(counters["hits"] - player.C0)/counters["overall"]}%')
        print(f'stand: {100*(counters["stands"] - player.C3)/counters["overall"]}%')
    
    save_model(player, RUN_NAME)
    run_test_phase(env, player)
